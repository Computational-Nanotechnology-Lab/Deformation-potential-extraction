...
6) Convert g(matrix elements from EPW) to M using the expression:
   𝑔_{𝜈𝑚⁢𝑛}⁡(𝐤,𝐪)= sqrt(ℏ/2⁢𝑚_0𝜔_{𝜈⁢𝐪})𝑀_{𝜈𝑚⁢𝑛}⁡(𝐤,𝐪), and plot the matrix elements.
   The code is present in g-M folder.
   Check which modes are polar.
8) Run the EPW calculation with tag "shortrange = .true." to subtract the long-range part of matrix elements. 
9) Compute the Acoustic deformation potential 
10) Compute the Optical deformation potential 
11) Identify coordinates of different valleys near VBM and CBM. Compute the Inter-valley deformation potential (the q-path for inter-valley deformation potential are such that k' = k+q) 
